...
const filtering = inventors.filter(function (inventors) {
  return inventors.year >= 1500 && inventors.year < 1600 ? console.table(inventors) : false;
});

// Array.prototype.map()
// 2. Give us an array of the inventors first and last names
const mapping = inventors.map(function (inventors) {
...
const lastestGroupby = people.sort(function(lastOne, nextOne){
  const [alast, afirst] = lastOne.split(', ');
  const [blast, bfirst] = lastOne.split(', ');
  return alast > blast ? -1 : 1;
});
console.log(lastestGroupby)
// 8. Reduce Exercise
